...
export const HOUSES = [
  ...
  { id: 1, name: '小木屋', req: { '木材': 14, '砖块': 8 }, capacity: 100, critRate: 0.05, coinBoost: 1.2, shovelsPerHour: 30, maxMiners: 2, minerSpeedBoost: 5 },
  { id: 2, name: '石屋', req: { '木材': 45, '砖块': 30, '水泥': 8 }, capacity: 200, critRate: 0.1, coinBoost: 1.5, shovelsPerHour: 60, maxMiners: 3, minerSpeedBoost: 10 },
  { id: 3, name: '小别墅', req: { '木材': 120, '砖块': 75, '水泥': 30, '玻璃': 8, '钢材': 3 }, capacity: 500, critRate: 0.15, coinBoost: 2, shovelsPerHour: 120, maxMiners: 5, minerSpeedBoost: 20 },
  { id: 4, name: '豪华别墅', req: { '砖块': 220, '水泥': 120, '玻璃': 40, '钢材': 15, '黑宝石': 10 }, capacity: 1000, critRate: 0.2, coinBoost: 3, shovelsPerHour: 240, maxMiners: 10, minerSpeedBoost: 30 },
  { id: 5, name: '庄园', req: { '水泥': 420, '玻璃': 210, '钢材': 75, '黑宝石': 30, '白宝石': 10 }, capacity: 2000, critRate: 0.25, coinBoost: 4, shovelsPerHour: 480, maxMiners: 15, minerSpeedBoost: 50 },
  { id: 6, name: '城堡', req: { '玻璃': 700, '钢材': 300, '白宝石': 30, '绿宝石': 5 }, capacity: 5000, critRate: 0.3, coinBoost: 5, shovelsPerHour: 960, maxMiners: 25, minerSpeedBoost: 80 },
  { id: 7, name: '王宫', req: { '玻璃': 1400, '钢材': 700, '绿宝石': 20, '红宝石': 5 }, capacity: 10000, critRate: 0.4, coinBoost: 10, shovelsPerHour: 1920, maxMiners: 40, minerSpeedBoost: 120 },
  { id: 8, name: '天空之城', req: { '钢材': 3000, '红宝石': 50, '蓝宝石': 10 }, capacity: 20000, critRate: 0.5, coinBoost: 15, shovelsPerHour: 3840, maxMiners: 60, minerSpeedBoost: 200 },
  { id: 9, name: '星际要塞', req: { '钢材': 7500, '蓝宝石': 30, '七彩宝石': 5 }, capacity: 50000, critRate: 0.6, coinBoost: 25, shovelsPerHour: 7680, maxMiners: 80, minerSpeedBoost: 300 },
  { id: 10, name: '宇宙神殿', req: { '钢材': 22000, '蓝宝石': 100, '七彩宝石': 20 }, capacity: 100000, critRate: 0.8, coinBoost: 50, shovelsPerHour: 15360, maxMiners: 100, minerSpeedBoost: 500 },
  { id: 11, name: '创世神域', req: { '钢材': 58000, '七彩宝石': 100 }, capacity: 250000, critRate: 1.0, coinBoost: 100, shovelsPerHour: 30720, maxMiners: 150, minerSpeedBoost: 800 },
  { id: 12, name: '维度枢纽', req: { '钢材': 145000, '七彩宝石': 300 }, capacity: 600000, critRate: 1.2, coinBoost: 200, shovelsPerHour: 61440, maxMiners: 200, minerSpeedBoost: 1200 },
  { id: 13, name: '永恒之境', req: { '钢材': 430000, '七彩宝石': 1000 }, capacity: 1500000, critRate: 1.5, coinBoost: 500, shovelsPerHour: 122880, maxMiners: 300, minerSpeedBoost: 2000 },
];

export const getHouseRequirementMultiplier = (targetHouseLevel: number) => {
  if (targetHouseLevel >= 11) return 1.35;
  if (targetHouseLevel >= 9) return 1.25;
  if (targetHouseLevel >= 7) return 1.18;
  if (targetHouseLevel >= 5) return 1.12;
  return 1;
};
...
const ACHIEVEMENT_REWARD_SCALE = 0.5;

export const ACHIEVEMENTS: Achievement[] = BASE_ACHIEVEMENTS.map((achievement) => ({
  ...achievement,
  rewardCoins: Math.max(100, Math.floor(achievement.rewardCoins * ACHIEVEMENT_REWARD_SCALE)),
}));
...
export const BASE_RATES = [
  ...
  { type: "rare", rate: 0.075 },
  { type: "epic", rate: 0.018 },
  { type: "legendary", rate: 0.005 },
  { type: "mythic", rate: 0.002 }
];

export const TIER_ITEMS: Record<DropTier, (GemType | MaterialType)[]> = {
  common: ['木材', '木材', '木材', '木材', '木材', '木材', '木材', '木材', '木材', '木材', '砖块', '砖块', '砖块', '砖块', '砖块', '砖块', '砖块', '砖块', '水泥', '水泥', '水泥', '黑宝石', '黑宝石', '黑宝石', '黑宝石', '黑宝石', '黑宝石', '白宝石', '白宝石', '白宝石', '白宝石', '银铲子碎片'],
  rare: ['红宝石', '红宝石', '绿宝石', '绿宝石', '银铲子碎片', '蓝宝石', '玻璃'],
  epic: ['蓝宝石', '蓝宝石', '金铲子碎片', '七彩铲子碎片', '红宝石', '绿宝石'],
  legendary: ['蓝宝石', '七彩铲子碎片', '红宝石', '钢材'],
  mythic: ['七彩宝石', '蓝宝石']
};
  ...
  adjusted.forEach(r => {
    ...
    items.forEach(item => {
      if (['木材', '砖块', '水泥', '玻璃', '钢材'].includes(item)) {
        mats[item] = (mats[item] || 0) + ratePerItem;
      } else {
        gems[item] = (gems[item] || 0) + ratePerItem;
      }
    });
  });
...
export const performDig = (state: GameState, isMiner: boolean = false): { result: DigResult, newPrdCounters: Record<string, number> } => {
  ...
  if (!isWarehouseFull) {
    ...
    if (!['木材', '砖块', '水泥', '玻璃', '钢材'].includes(droppedItem)) {
      if (tier === 'common') {
        if (Math.random() < 0.2) finalItem = Math.random() < 0.65 ? '木材' : '砖块';
      } else if (tier === 'rare') {
        if (Math.random() < 0.15) finalItem = Math.random() < 0.7 ? '砖块' : '水泥';
      } else if (tier === 'epic') {
        if (Math.random() < 0.08) finalItem = '水泥';
      }
    }

    if (finalItem === '玻璃' && Math.random() < 0.25) {
      finalItem = '水泥';
    }
    if (finalItem === '钢材' && Math.random() < 0.35) {
      finalItem = Math.random() < 0.6 ? '砖块' : '水泥';
    }
...
export const MINER_TIERS = [
  { level: 1, name: '普通矿工', digsPerMinute: 0.12, upgradeCost: 3000 },
  { level: 2, name: '熟练矿工', digsPerMinute: 0.25, upgradeCost: 15000 },
  { level: 3, name: '专家矿工', digsPerMinute: 0.6, upgradeCost: 70000 },
  { level: 4, name: '大师矿工', digsPerMinute: 1.2, upgradeCost: 320000 },
  { level: 5, name: '传奇矿工', digsPerMinute: 2.4, upgradeCost: 1500000 },
  { level: 6, name: '神话矿工', digsPerMinute: 4.8, upgradeCost: 6500000 },
  { level: 7, name: '虚空矿工', digsPerMinute: 9.5, upgradeCost: 30000000 },
  { level: 8, name: '混沌矿工', digsPerMinute: 18, upgradeCost: 140000000 },
];

export const getMinerSpeedPenalty = (minerLevel: number) => {
  if (minerLevel >= 8) return 0.72;
  if (minerLevel >= 7) return 0.76;
  if (minerLevel >= 6) return 0.8;
  if (minerLevel >= 5) return 0.86;
  if (minerLevel >= 4) return 0.92;
  return 1;
};

export const getEffectiveMinerDigsPerMinute = (minerLevel: number, baseDigsPerMinute: number) => {
  return baseDigsPerMinute * getMinerSpeedPenalty(minerLevel);
};

export const getMinerUpgradeCostMultiplier = (targetMinerLevel: number, houseLevel: number) => {
  const levelPressure = targetMinerLevel >= 7 ? 1.28 : targetMinerLevel >= 6 ? 1.2 : targetMinerLevel >= 5 ? 1.14 : targetMinerLevel >= 4 ? 1.08 : 1;
  const housePressure = houseLevel >= 11 ? 1.15 : houseLevel >= 9 ? 1.11 : houseLevel >= 7 ? 1.08 : houseLevel >= 5 ? 1.05 : 1;
  return levelPressure * housePressure;
};
